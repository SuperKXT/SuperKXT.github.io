      ...
      {			
        ...
        $.get('portfolio/portfolio.xml', function(file){
			$(file).find('item').each(function(){
				...
				htm += "<div class='itemImage'> <span class='thumbnail " + id + "'></span></div>";
				htm += "<div class='itemHeading'>" + name.text() + "</div>";
				htm += "<div class='itemDescription'>" + description.text() + "</div> </div>";
		...
		$(window).resize(function() {
			if($("#popUp").css("display") == "block" && $("#popUp").height() < $(document).height()) {
				$("#popUp").height($(document).height());
			}
		});
	});
